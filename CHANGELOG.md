## 23.07.2026 — Cyan 26.8.0

```diff
[improvements]
+ Professional design system: added five curated built-in themes (Cyan Pro, Midnight, Aurora, Graphite, Bloom) with high-contrast, mobile-friendly palettes.
...
+ Add-on controls (ESP, Aimbot, Radar, Movement, Visuals, Camera, Server Info, Protections, QoL, UI Utilities) are visible in the "UI Settings" tab with no KeySystem gating.

## 23.07.2026 — Cyan 26.7.8

```diff
[fixes]
+ Made the example showcase self-contained: it now prefers the local sibling Library and add-ons when present and only falls back to the published `main` branch over HTTP, so a cloned repo runs fully offline with the exact local (ungated) code and shows every add-on.
+ Hardened add-on visibility so ESP and all add-on controls render in the "UI Settings" tab by default with no KeySystem gating.

## 23.07.2026 — Cyan 26.7.7

```diff
[fixes]
+ Fixed the mobile circular launcher button so it can be dragged to a new position; once moved, viewport/orientation changes no longer reset it.
+ Fixed the example showcase so the "UI Settings" tab (which holds ESP and every add-on control) is visible by default instead of being hidden behind the KeySystem demo gate.

## 22.07.2026 — Cyan 26.7.6

```diff
[changes]
+ Bumped the package version to 26.7.6.
+ Added a canonical StyLua configuration (stylua.toml, 4-space indentation) and normalized all sources to it, resolving the mixed tabs/spaces indentation across the library and add-ons.
+ Fixed an invalid tuple return type in the ESP add-on: `(Vector3, Vector3)?` is not valid Luau; changed to `(Vector3, Vector3)`.
+ Fixed four malformed Unicode escape sequences in Example.lua: `\u00B0` changed to the valid Luau form `\u{00B0}` (degree sign and `°/s` suffix).

## 21.07.2026 — Cyan 26.7.5
...
+ Added a configurable Liquid Glass menu style with translucent Cyan surfaces, moving background reflections, animated sheen, accent strokes, optional blur, and Liquid/Crystal/Frosted/Ocean/Aurora/Midnight/Solid presets.
