...
+ Added the bundled CyanLogo asset and applied it to the example window and default loading-screen branding.
...
+ Improved the example Experience Status HUD: it now stays hidden before login, reflects verification/readiness/session progress, and includes a post-login visibility setting.
+ Simplified mobile controls: a 56px circular Toggle opens the menu, then becomes Lock/Unlock while open, with a mobile close header action.
...
+ Mobile windows now fit the active viewport and refresh after portrait/landscape or camera viewport changes.
