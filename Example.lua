...
-- (which would otherwise blank out every control defined after it). If an
-- addon fails to load we keep going and let SafeSetup report it later.
...
local Window = Library:CreateWindow({
    ...
    Resizable = false, -- fixed size; individual options stay toggleable
    AutoShow = false, -- hidden until the key system verifies access
})

-- Keep the menu hidden until the key system verifies access (see the
-- presentation flow at the end of the script). This stops the interface from
-- flashing in before the user is authenticated.
...
local function SetupESP()
    ...
    local AimGroup = Tabs.Combat:AddRightGroupbox("Aimbot", "crosshair")
...
local function SetupUIUtilities()
    ...
    local UIGroup = Tabs.Misc:AddLeftGroupbox("UI Utilities", "palette")
...
local function SetupWeaponMods()
    ...
    local WeaponGroup = Tabs.Combat:AddRightGroupbox("Weapon Mods", "swords")
...
-- failed and the error it hit on your game.
do
...
--// =====================================================================
--//  PRESENTATION FLOW
--//  Loading screen  ->  Key System (dedicated window)  ->  Main menu
--//  The main menu stays hidden until the key is verified, so options can
--//  never appear before authentication. If anything here fails, the menu
--//  is revealed directly so the user is never stuck on a blank screen.
--// =====================================================================
...
local function RunPresentation()
    -- 1) Lightweight loading screen (prevents a blank flash before the UI)
    ...
    local Key = LoadedKeySystem.new({
        Validate = function(Key)
            return Key == ACCESS_KEY, "Invalid access key"
        end,
        MaxAttempts = 5,
        OnVerified = function()
            -- Close the key window, then reveal the main menu.
            pcall(function()
                if KeyWindow.Destroy then
                    KeyWindow:Destroy()
                elseif KeyWindow.Holder then
                    KeyWindow.Holder.Visible = false
                elseif KeyWindow.Container then
                    KeyWindow.Container.Visible = false
                end
            end)
            Library:Toggle(false)
            Library:Toggle(true)
    ...
    -- Keep the main menu hidden while the key window is open.
    Library:Toggle(false)
end
