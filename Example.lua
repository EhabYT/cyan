...
-- Experience status is intentionally hidden until the access flow is verified.
local StatusHUD
local StatusAccess
local StatusReadiness
local StatusTimer
local StatusObjectives
...
local DemoKeySystem = KeySystem.new({
    Validate = function(ReceivedKey)
        if ReceivedKey == "demo-access" then
    ...
    OnVerified = function()
        ...
        if StatusAccess then
            StatusAccess:SetValue("Verified")
        end
        if StatusReadiness then
            StatusReadiness:SetValue(1)
        end
        if StatusObjectives then
            StatusObjectives:SetCount(1)
        end
        if StatusTimer then
            StatusTimer:Reset()
        end
        local StatusToggle = Library.Toggles.ShowExperienceStatus
...
DemoKeySystem:Attach(Tabs.Key, {
    Prompt = "Enter `demo-access` to test the callback-driven key system.",
    Placeholder = "Access key",
})

-- Compact experience dashboard. It becomes visible only after a successful login.
StatusHUD = HUD.new(Library, {
    Title = "Cyan Experience",
    Position = UDim2.fromOffset(12, 80),
    Visible = false,
})

StatusHUD:AddText("Version", {
    Text = "Cyan",
    Value = "v" .. Library.Version,
})
StatusAccess = StatusHUD:AddText("Access", {
    Text = "Access",
    Value = "Login required",
})
StatusReadiness = StatusHUD:AddProgress("Readiness", {
    Text = "Session readiness",
    Value = 0.25,
})
StatusObjectives = StatusHUD:AddCounter("Objectives", {
    Text = "Objectives",
    Value = 0,
})
StatusTimer = StatusHUD:AddTimer("SessionTimer", {
    Text = "Session time",
    Duration = 900,
    Running = false,
})
StatusHUD:AddProgressRing("Milestone", {
    Text = "Milestone",
    Value = 0.6,
})
StatusHUD:AddPerformance("Performance", {
    Text = "Performance",
})
...
MenuGroup:AddToggle("ShowExperienceStatus", {
    Text = "Show Experience Status",
    Default = false,
    Tooltip = "The status panel is available after a successful login.",
    Callback = function(Value)
        if DemoKeySystem:IsVerified() then
            StatusHUD:SetVisible(Value)
        end
    end,
})
