...
-- Loads a module over HttpGet and compiles/runs it inside a pcall so that a
-- single failed download or module error can never abort the whole script
-- (which would otherwise blank out every control defined after it).
...
--[[
    SafeSetup wraps an addon's UI/setup block in pcall so that a single
    runtime error in one addon can never blank out the rest of the menu.
    This directly fixes "some options show, some don't" reports: if an
    addon throws while building its controls, the others keep loading and a
    notification is shown instead of the whole menu silently dying.
]]
-- Tracks each addon's load/build outcome so the Diagnostics panel can show
-- exactly which addon (if any) failed, instead of leaving you guessing why
...
local Window = Library:CreateWindow({
    ...
    Center = true, -- static, centered menu (does not drift around the screen)
...
local UISettingsTab = Tabs.Settings
...
local function SetupESP()
    ...
    ESPGroup:AddToggle("ESPHealthBar", {
        ...
        Default = true,
        Callback = function(Value)
            ESPManager:SetHealthBarsVisible(Value)
        end,
    })
    ...
    ESPGroup:AddToggle("ESPDistance", {
        ...
        Default = true,
        Callback = function(Value)
            ESPManager:SetDistanceVisible(Value)
        end,
    })
    ...
    ESPGroup:AddToggle("ESPChams", {
        Text = "Chams",
        Default = false,
        Tooltip = "Highlights player characters through walls.",
        Callback = function(Value)
            ESPManager:SetChamsVisible(Value)
        end,
    })
    ...
    ESPGroup:AddToggle("ESPSkeleton", {
        Text = "Skeleton / Bones",
        Default = false,
        Tooltip = "Draws bone lines between joints (R15 + R6 rigs).",
        Callback = function(Value)
            ESPManager:SetSkeletonVisible(Value)
        end,
    })
    ...
    ESPGroup:AddToggle("ESPWeapon", {
        Text = "Weapon ESP",
        Default = false,
        Tooltip = "Shows the target's current tool/weapon name above the box.",
        Callback = function(Value)
            ESPManager:SetWeaponVisible(Value)
        end,
    })

    ESPGroup:AddToggle("ESPViewLine", {
        Text = "View / Aim Line",
        Default = false,
        Tooltip = "Draws a line from screen-center to the target's box (look-at indicator).",
        Callback = function(Value)
            ESPManager:SetViewLineVisible(Value)
        end,
    })
    ...
    ESPGroup:AddToggle("ESPRainbow", {
        Text = "Rainbow Box",
        Default = false,
        Tooltip = "Cycles the box color through the rainbow automatically.",
        Callback = function(Value)
            ESPManager:SetRainbow(Value)
        end,
    })
    ...
    AimGroup:AddSlider("AimbotHitboxSize", {
        Text = "Hitbox Size",
        Default = 0,
        Min = 0,
        Max = 10,
        Rounding = 1,
        Tooltip = "Expands the aim point with a small random offset (hitbox extender).",
        Callback = function(Value)
            ESPManager:SetHitboxSize(Value)
        end,
    })
    ...
    task.delay(2, function()
        ...
        ESPManager:AddPlayerESP({
            Box = true,
            BoxStyle = "Corner",
            HealthBar = Toggles.ESPHealthBar.Value,
            Name = true,
            Distance = Toggles.ESPDistance.Value,
            Tracer = false,
            Chams = Toggles.ESPChams.Value,
            Skeleton = Toggles.ESPSkeleton.Value,
            Weapon = Toggles.ESPWeapon.Value,
            ViewLine = Toggles.ESPViewLine.Value,
            TeamCheck = false,
        })
...
local function SetupMovement()
    ...
    MoveGroup:AddToggle("InfiniteJumpToggle", {
        Text = "Infinite Jump",
        Default = false,
        Tooltip = "Lets you jump again while airborne.",
        Callback = function(Value)
            MovementManager:SetInfiniteJump(Value)
        end,
    })

    MoveGroup:AddToggle("AirControlToggle", {
        Text = "Air Control",
        Default = false,
        Tooltip = "Lets you steer horizontally while falling.",
        Callback = function(Value)
            MovementManager:SetAirControl(
                Value,
                Options.AirControlSpeed and Options.AirControlSpeed.Value or 50
            )
        end,
    })

    MoveGroup:AddSlider("AirControlSpeed", {
        Text = "Air Control Speed",
        Default = 50,
        Min = 10,
        Max = 200,
        Rounding = 0,
        Suffix = " studs/s",
        Callback = function(Value)
            if Toggles.AirControlToggle and Toggles.AirControlToggle.Value then
                MovementManager:SetAirControl(true, Value)
            end
        end,
    })

    MoveGroup:AddToggle("SpinToggle", {
        Text = "Spin",
        Default = false,
        Tooltip = "Continuously rotates your character.",
        Callback = function(Value)
            MovementManager:SetSpin(Value, Options.SpinSpeed and Options.SpinSpeed.Value or 30)
        end,
    })

    MoveGroup:AddSlider("SpinSpeed", {
        Text = "Spin Speed",
        Default = 30,
        Min = 5,
        Max = 360,
        ...
        Callback = function(Value)
            if Toggles.SpinToggle and Toggles.SpinToggle.Value then
                MovementManager:SetSpin(true, Value)
            end
        end,
    })
    ...
    MoveGroup:AddButton({
        Text = "Blink",
        Tooltip = "Dashes you forward by the Blink Distance.",
        Callback = function()
            MovementManager:Blink(Options.BlinkDistance and Options.BlinkDistance.Value or 25)
        end,
    })

    MoveGroup:AddSlider("BlinkDistance", {
        Text = "Blink Distance",
        Default = 25,
        Min = 5,
        Max = 200,
        ...
        Suffix = " studs",
        Callback = function(Value)
            MovementManager:SetBlinkDistance(Value)
        end,
    })

    MoveGroup:AddToggle("ClickTeleportToggle", {
        Text = "Click Teleport",
        Default = false,
        Tooltip = "Teleports you to where you click (left/right mouse).",
        Callback = function(Value)
            MovementManager:SetClickTeleport(Value)
        end,
    })

    MoveGroup:AddToggle("GravityToggle", {
        Text = "Override Gravity",
        Default = false,
        Tooltip = "Sets a custom workspace gravity.",
        Callback = function(Value)
            if Value then
                MovementManager:SetGravity(
                    Options.GravitySlider and Options.GravitySlider.Value or 196.2
                )
            else
                MovementManager:ResetGravity()
            end
        end,
    })

    MoveGroup:AddSlider("GravitySlider", {
        Text = "Gravity",
        Default = 196.2,
        Min = 0,
        Max = 500,
        Rounding = 1,
        Callback = function(Value)
            if Toggles.GravityToggle and Toggles.GravityToggle.Value then
                MovementManager:SetGravity(Value)
            end
        end,
    })
...
local function SetupVisuals()
    ...
    VisGroup:AddDropdown("SkyboxPreset", {
        Text = "Skybox",
        Values = { "Default", "Night", "Brick" },
        Default = "Default",
        Tooltip = "Swaps the world skybox (best-effort; 'Default' restores the place sky).",
        Callback = function(Value)
            VisualsManager:SetSkybox(Value)
        end,
    })

    VisGroup:AddSlider("TimeOfDay", {
        Text = "Time of Day",
        Default = 14,
        Min = 0,
        Max = 24,
        ...
        Suffix = "h",
        Callback = function(Value)
            VisualsManager:SetClockTime(Value)
        end,
    })

    VisGroup:AddLabel("Ambient"):AddColorPicker("AmbientColor", {
        Default = Color3.fromRGB(128, 128, 128),
        Title = "Ambient color",
        Callback = function(Value)
            VisualsManager:SetAmbient(Value)
        end,
    })

    VisGroup:AddToggle("NoFogToggle", {
        Text = "No Fog",
        Default = false,
        Tooltip = "Removes distance fog entirely.",
        Callback = function(Value)
            VisualsManager:SetNoFog(Value)
        end,
    })
...
local function SetupCamera()
    ...
    CamGroup:AddToggle("FirstPersonToggle", {
        Text = "First Person",
        Default = false,
        Tooltip = "Snaps the camera to your character's head.",
        Callback = function(Value)
            CameraManager:SetFirstPerson(Value)
        end,
    })

    CamGroup:AddToggle("ZoomToggle", {
        Text = "Zoom",
        Default = false,
        Tooltip = "Lowers FOV for a scoped / zoomed view.",
        Callback = function(Value)
            CameraManager:SetZoom(Value, Options.ZoomLevel and Options.ZoomLevel.Value or 40)
        end,
    })

    CamGroup:AddSlider("ZoomLevel", {
        Text = "Zoom Level",
        Default = 40,
        Min = 20,
        Max = 80,
        Rounding = 0,
        Callback = function(Value)
            if Toggles.ZoomToggle and Toggles.ZoomToggle.Value then
                CameraManager:SetZoom(true, Value)
            end
        end,
    })

    CamGroup:AddToggle("LockCameraToggle", {
        Text = "Lock Camera",
        Default = false,
        Tooltip = "Locks the camera to where you were looking when enabled.",
        Callback = function(Value)
            CameraManager:SetViewLock(Value)
        end,
    })
end
...
local function SetupProtections()
    ...
    ProtGroup:AddToggle("AntiBanToggle", {
        Text = "Anti Ban",
        Default = false,
        Tooltip = "Enables Anti Kick (with rejoin) and Anti Idle together for maximum protection.",
        Callback = function(Value)
            ProtectionManager:SetAntiKick(Value, Value)
            ProtectionManager:SetAntiIdle(Value)
        end,
    })
...
local function SetupQoL()
    ...
    QoLGroup:AddButton({
        Text = "Copy Job ID",
        Tooltip = "Copies the current server JobId to clipboard.",
        Callback = function()
            local JobId = QoLManager:CopyJobId()
            Library:Notify({ Description = "Job ID copied:\n" .. tostring(JobId), Time = 5 })
        end,
    })

    QoLGroup:AddToggle("ChatSpyToggle", {
        Text = "Chat Spy",
        Default = false,
        Tooltip = "Shows every chat message (including whispers) as a notification.",
        Callback = function(Value)
            QoLManager:SetChatSpy(Value)
        end,
    })
    ...
    QoLGroup:AddToggle("AutoRejoinToggle", {
        Text = "Auto Rejoin",
        Default = false,
        Tooltip = "Automatically rejoins the server if you get disconnected.",
        Callback = function(Value)
            QoLManager:SetAutoRejoin(Value)
        end,
    })
    ...
    QoLGroup:AddLabel("Notifications")
    QoLGroup:AddToggle("PlayerToastsToggle", {
        Text = "Player Join/Leave Toasts",
        Default = false,
        Tooltip = "Shows a toast when a player joins or leaves the server.",
        Callback = function(Value)
            QoLManager:SetNotificationsEnabled(Value)
        end,
    })

    QoLGroup:AddSlider("ToastDuration", {
        Text = "Toast Duration",
        Default = 5,
        Min = 1,
        Max = 30,
        ...
        Suffix = "s",
        Callback = function(Value)
            QoLManager:SetNotificationDuration(Value)
        end,
    })

    QoLGroup:AddDivider()

    QoLGroup:AddLabel("Performance (best-effort)")
    QoLGroup:AddSlider("GraphicsQuality", {
        Text = "Graphics Quality",
        Default = 10,
        Min = 1,
        Max = 10,
        Rounding = 0,
        Tooltip = "Lowers rendering quality for more FPS (engine/executor dependent).",
        Callback = function(Value)
            QoLManager:SetGraphicsQuality(Value)
        end,
    })

    QoLGroup:AddLabel(
        "FPS cap / unlocker needs an executor-specific API; not possible in pure Luau."
    )
end
