...
function Library:SetText(Key: string, Text: string)
    ...
    for Index = #Bindings, 1, -1 do
        ...
        if Binding and Binding.Connected then
            local Success = pcall(Binding.Callback, Text)
            if not Success then
...
function Library:CreateWindow(WindowInfo)
    ...
    local function EnsureCommandPalette()
        ...
        Library:BindSystemText("CommandSearchPlaceholder", function(Value)
            if CommandSearchBox and CommandSearchBox.Parent then
    ...
    if Library.IsMobile then
        ...
        local function PositionMobileControls()
            local SafeTopLeft, SafeBottomRight = GetSafeAreaInsets()
            local EdgePadding = 6
