...
function Library:CreateWindow(WindowInfo)
    ...
    local MobileControlButton
    local SetMobileControlText
    ...
    function Window:Toggle(Value: boolean?)
        ...
        if SetMobileControlText then
            SetMobileControlText(if Library.Toggled then (Library.CantDragForced and "Unlock" or "Lock") else "Toggle")
        end
    ...
    if Library.IsMobile then
        -- One mobile control: it opens the menu while closed, then becomes Lock/Unlock while open.
        MobileControlButton = Library:AddDraggableButton("Toggle", function()
            if not Library.Toggled then
                Library:Toggle(true)
                return
            end
            ...
            SetMobileControlText(Library.CantDragForced and "Unlock" or "Lock")
        end, true, true)

        -- Keep the mobile control circular and large enough for touch interaction.
        local MobileButton = MobileControlButton.Button
        local MobileButtonSize = 56
        local MobileCorner = MobileButton:FindFirstChildOfClass("UICorner")
        if MobileCorner then
            MobileCorner.CornerRadius = UDim.new(1, 0)
        end
        Library:AddToRegistry(MobileButton, {
            BackgroundColor3 = "AccentColor",
            TextColor3 = "BackgroundColor",
        })
        MobileButton.BackgroundColor3 = Library.Scheme.AccentColor
        MobileButton.TextColor3 = Library.Scheme.BackgroundColor
        ...
        SetMobileControlText = function(Text: string)
            MobileControlButton:SetText(Text)
            MobileButton.Size = UDim2.fromOffset(MobileButtonSize, MobileButtonSize)

            local IconName = if Text == "Toggle" then "menu" else (Text == "Lock" and "lock" or "lock-open")
            local Icon = Library:GetIcon(IconName)
            if Icon then
                MobileIcon.Image = Icon.Url
                MobileIcon.ImageRectOffset = Icon.ImageRectOffset
                MobileIcon.ImageRectSize = Icon.ImageRectSize
        ...
        SetMobileControlText("Toggle")

        local function PositionMobileControl()
            local SafeTopLeft, SafeBottomRight = GetSafeAreaInsets()
            local EdgePadding = 6
            if WindowInfo.MobileButtonsSide == "Right" then
                MobileControlButton.Button.AnchorPoint = Vector2.new(1, 0)
                MobileControlButton.Button.Position =
                    UDim2.new(1, -(SafeBottomRight.X + EdgePadding), 0, SafeTopLeft.Y + EdgePadding)
            else
                MobileControlButton.Button.AnchorPoint = Vector2.new(0, 0)
                MobileControlButton.Button.Position =
        ...
        -- The close action keeps the single-control mobile flow usable after it becomes Lock.
        ...
        if WindowInfo.ShowMobileButtons == false then
            MobileControlButton.Button.Visible = false
        end
