...
Cyan is a maintained, typed Luau UI library for Roblox. It provides windows, tabs, groupboxes, inputs, notifications, dialogs, theming, optional configuration persistence, a cyan/navy default palette, and a polished key-gated settings example.

**Current package version:** `26.7.37`
**License:** [MIT](LICENSE)
...
![Cyan rounded Liquid Glass interface preview](assets/Example.png)
...
- `Window:AddCommand()` creates searchable actions for the built-in command palette; open it with **Ctrl+P** or `Window:OpenCommandPalette()`. Returned commands support `SetText()`, `SetDescription()`, `SetKeywords()`, and `SetOrder()` for live updates or localization.
...
Cyan includes keyboard-first search (`Ctrl+K`), Escape-aware text handling, DPI scaling, custom cursor control, and a reduced-motion preference. On touch devices, the menu automatically fits the current viewport, starts with a compact sidebar by default, supports optional horizontal swipe navigation between visible tabs, respects the Roblox safe area for notches and system bars, and refreshes after portrait/landscape changes. Configure this with `WindowInfo.MobileScale`, `WindowInfo.MobileViewportPadding`, `WindowInfo.MobileStartCompacted`, `WindowInfo.MobileLayout`, and `WindowInfo.MobileLockLongPressDuration` (0.25–2 seconds; default 0.55). Runtime presets are available through `Window:SetMobileLayout("Compact" | "Balanced" | "Expanded")`. Header actions receive larger touch targets, while optional desktop quick actions can be hidden on mobile. The bundled CyanLogo brands the primary mobile menu button. The 52px circular button opens or closes the menu directly on consecutive taps and can be dragged to a preferred on-screen position (`MobileActionsDraggable = true` by default). Press and hold it to reveal or hide the optional Lock/Unlock action; normal taps never open the lock panel. The lock action dismisses after use or when the user taps outside it, and the controls use CyanLogo, lock, and unlock icons when available. Use `Library:SetReducedMotion(true)` for users who prefer fewer transitions; call it with `false` to restore the window’s configured animations.
...
`Localization` provides dictionaries, locale fallback, `{parameter}` interpolation, RTL support, locale-change callbacks, and live text bindings. `Library:SetLocalization()` also translates Cyan's supported built-in text at runtime. See [docs/LOCALIZATION.md](docs/LOCALIZATION.md) for examples.
...
Both managers now use `CyanLibSettings` as their default root folder. Save Manager includes validated config renaming with autoload migration, and Theme Manager includes a stable **Cyan** preset so the cyan/navy palette can always be restored after customizing the mutable `Default` theme. Both generated panels can share a `Localization` instance through `SetLocalization()`; see [docs/LOCALIZATION.md](docs/LOCALIZATION.md). See [docs/MIGRATION.md](docs/MIGRATION.md) when moving from Obsidian.
...
stylua Library.lua Library.d.luau Example.lua init.luau addons tests/keysystem_smoke.luau tests/localization_smoke.luau

# Verify formatting and syntax without changing source.
stylua --check Library.lua Library.d.luau Example.lua init.luau addons tests/keysystem_smoke.luau tests/localization_smoke.luau
...
# Smoke-test dictionaries, fallback, RTL, and live text bindings.
luau tests/localization_smoke.luau
...
- [Key-gated settings example](Example.lua)
