...
function ESP:AddPlayerESP(Options: PlayerESPOptions)
    ...
    local function UpdateESPForPlayer(Player: Player)
        ...
        if self.Rainbow then
            BoxColor = Color3.fromHSV((tick() * 0.15) % 1, 1, 1)
        elseif self.BoxColorOverride then
            BoxColor = self.BoxColorOverride
        elseif not BoxColor then
            ...
            if BottomScreen and TopScreen then
                ...
                if
                    ...
                    and (
                        ...
                        or self:IsTargetInFOV(Player, self._AimbotOptions.FOV or 60)
                    )
                then
                    local Viewport = workspace.CurrentCamera
                            and workspace.CurrentCamera.ViewportSize
                        or Vector2.new(1920, 1080)
                    Handler:DrawViewLine(Viewport.X / 2, Viewport.Y / 2, CenterX, TopY, BoxColor)
                end
                ...
                if Opts.Skeleton then
                    Handler:DrawSkeleton(Character, BoxColor, Opts.BoxTransparency or 0)
                end
            end
        end
    ...
    function Handler:DrawWeapon(Text: string, CenterX: number, TopY: number, Color: Color3)
        if not self.WeaponLabel or not self.WeaponLabel.Parent then
            if self.WeaponLabel then
                self.WeaponLabel:Destroy()
            end
            self.WeaponLabel = Instance.new("TextLabel")
            self.WeaponLabel.BackgroundTransparency = 1
            self.WeaponLabel.BorderSizePixel = 0
            self.WeaponLabel.FontFace = Font.fromEnum(Enum.Font.Code)
            self.WeaponLabel.RichText = true
            self.WeaponLabel.TextStrokeTransparency = 0.5
            self.WeaponLabel.ZIndex = 9999
            self.WeaponLabel.Parent = self.Library.ScreenGui
        end
        self.WeaponLabel.Text = Text
        self.WeaponLabel.TextColor3 = Color
        self.WeaponLabel.TextSize = 11
        self.WeaponLabel.Size = UDim2.fromOffset(200, 14)
        self.WeaponLabel.Position = UDim2.fromOffset(CenterX - 100, TopY)
        self.WeaponLabel.TextXAlignment = Enum.TextXAlignment.Center
    end

    function Handler:DrawViewLine(
        StartX: number,
        StartY: number,
        EndX: number,
        EndY: number,
        ...
        if not self.ViewLineFrame or not self.ViewLineFrame.Parent then
            if self.ViewLineFrame then
                self.ViewLineFrame:Destroy()
            end
            self.ViewLineFrame = Instance.new("Frame")
            self.ViewLineFrame.BorderSizePixel = 0
            self.ViewLineFrame.ZIndex = 9997
            self.ViewLineFrame.Parent = self.Library.ScreenGui
        end
        local DX = EndX - StartX
        local DY = EndY - StartY
        local Length = math.sqrt(DX * DX + DY * DY)
        self.ViewLineFrame.BackgroundColor3 = Color
        self.ViewLineFrame.BackgroundTransparency = 0
        self.ViewLineFrame.Size = UDim2.fromOffset(math.max(Length, 1), 1)
        self.ViewLineFrame.Position = UDim2.fromOffset(StartX, StartY)
        self.ViewLineFrame.Rotation = math.deg(math.atan2(DY, DX))
    end
    ...
    function Handler:DrawSkeleton(Character: Model, Color: Color3, Transparency: number)
        local BONES = {
            { "Head", "UpperTorso" },
        ...
        for _, Pair in BONES do
            ...
            if A and B and A:IsA("BasePart") and B:IsA("BasePart") then
                local SA = WorldToScreen(A:GetPivot().Position)
                local SB = WorldToScreen(B:GetPivot().Position)
                if SA and SB then
                    idx = idx + 1
                    self:_DrawBoneLine(idx, SA.X, SA.Y, SB.X, SB.Y, Color, Transparency)
                end
            end
        end

        for i = idx + 1, #self.SkeletonLines do
            if self.SkeletonLines[i] then
                self.SkeletonLines[i]:Destroy()
                self.SkeletonLines[i] = nil
            end
        end
    end

    function Handler:_DrawBoneLine(
        Index: number,
        StartX: number,
        StartY: number,
        EndX: number,
        ...
        if not self.SkeletonLines[Index] or not self.SkeletonLines[Index].Parent then
            if self.SkeletonLines[Index] then
                self.SkeletonLines[Index]:Destroy()
            end
            local Line = Instance.new("Frame")
            Line.BorderSizePixel = 0
            Line.ZIndex = 9998
            Line.Parent = self.Library.ScreenGui
            self.SkeletonLines[Index] = Line
        end
        local DX = EndX - StartX
        local DY = EndY - StartY
        local Length = math.sqrt(DX * DX + DY * DY)
        ...
        Line.BackgroundColor3 = Color
        Line.BackgroundTransparency = Transparency
        Line.Size = UDim2.fromOffset(math.max(Length, 1), 1)
        Line.Position = UDim2.fromOffset(StartX, StartY)
...
function ESP:SetHealthBarsVisible(Visible: boolean)
    self.HealthBarsVisible = Visible
    for _, Handler in self.Players do
        if not Handler.Destroyed and Handler.Options then
            Handler.Options.HealthBar = Visible
        end
    end
end

function ESP:SetDistanceVisible(Visible: boolean)
    self.DistanceVisible = Visible
    for _, Handler in self.Players do
        if not Handler.Destroyed and Handler.Options then
            Handler.Options.Distance = Visible
        end
    end
end

function ESP:SetChamsVisible(Visible: boolean)
    self.ChamsVisible = Visible
    for _, Handler in self.Players do
        if not Handler.Destroyed and Handler.Options then
            Handler.Options.Chams = Visible
        end
    end
end

function ESP:SetSkeletonVisible(Visible: boolean)
    self.SkeletonVisible = Visible
    for _, Handler in self.Players do
        if not Handler.Destroyed and Handler.Options then
            Handler.Options.Skeleton = Visible
        end
    end
end

function ESP:SetBoxColor(Color: Color3)
    self.BoxColorOverride = Color
end

function ESP:SetRainbow(Enabled: boolean)
    self.Rainbow = Enabled
end

function ESP:SetWeaponVisible(Visible: boolean)
    for _, Handler in self.Players do
        if not Handler.Destroyed and Handler.Options then
            Handler.Options.Weapon = Visible
        end
    end
end

function ESP:SetViewLineVisible(Visible: boolean)
    for _, Handler in self.Players do
        if not Handler.Destroyed and Handler.Options then
            Handler.Options.ViewLine = Visible
        end
    end
end

function ESP:SetHitboxSize(Size: number)
    self._AimbotOptions.HitboxSize = Size
end
...
function ESP:GetAimbotTargetPosition(Player: Player): Vector3?
    ...
    local HitPos = GetHitboxPosition(Character, self._AimbotOptions.Hitbox)
    local Size = self._AimbotOptions.HitboxSize
    if HitPos and Size and Size > 0 then
        HitPos = HitPos
