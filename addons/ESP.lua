...
local function IsPlayerVisible(Target: Model): boolean
	...
	local Result = workspace:Raycast(Origin, Direction * (RootPart.Position - Origin).Magnitude, RaycastParams)
	return Result == nil
end
...
local function GetCharacterBoundingBox(Character: Model, IgnoreHead: boolean?): (Vector3, Vector3)?
	...
	local Bottom = RootPart.CFrame:PointToWorldSpace(Vector3.new(-Size.X / 2, -Size.Y / 2, -Size.Z / 2))
	local Top = RootPart.CFrame:PointToWorldSpace(Vector3.new(Size.X / 2, Size.Y / 2 + HeadOffset, Size.Z / 2))
...
function ESP:AddPlayerESP(Options: PlayerESPOptions)
		...
		if Bottom and Top then
			...
			if BottomScreen and TopScreen then
				...
				if Opts.Box ~= false then
					Handler:DrawBox(LeftX, TopY, Width, Height, BoxColor, Opts.BoxStyle or "Corner", Opts.BoxTransparency or 0, Opts.BoxThickness or 1)
				end
				...
				if Opts.HealthBar ~= false then
					...
					Handler:DrawHealthBar(LeftX - 6, TopY, 4, Height, HealthPercent, HealthColor, Opts.HealthBarTransparency or 0)
				end

				if Opts.Name ~= false then
					Handler:DrawName(Player.Name, CenterX, TopY - 14, Opts.NameSize or 13, Opts.NameColor or Color3.fromRGB(255, 255, 255))
				end
				...
				if Opts.Distance ~= false then
					...
					Handler:DrawDistance(DistanceText, CenterX, BottomScreen.Y + 4, Opts.DistanceColor or Color3.fromRGB(200, 200, 200))
				end
				...
				if Opts.Tracer then
					...
					Handler:DrawTracer(StartX, StartY, CenterX, BottomScreen.Y, Opts.TracerColor or BoxColor, Opts.TracerTransparency or 0)
				end
			end
		end
	...
	function Handler:DrawBox(Left: number, Top: number, Width: number, Height: number, Color: Color3, Style: BoxStyle, Transparency: number, Thickness: number)
		...
		if StyleStr == "Standard" then
			MakeLine(1, { Position = UDim2.fromOffset(Left, Top), Size = UDim2.fromOffset(Width, Thickness) })
			MakeLine(2, { Position = UDim2.fromOffset(Left, Top + Height), Size = UDim2.fromOffset(Width, Thickness) })
			MakeLine(3, { Position = UDim2.fromOffset(Left, Top), Size = UDim2.fromOffset(Thickness, Height) })
			MakeLine(4, { Position = UDim2.fromOffset(Left + Width, Top), Size = UDim2.fromOffset(Thickness, Height) })
		elseif StyleStr == "Outline" then
			...
			MakeLine(1, { Position = UDim2.fromOffset(Left, Top), Size = UDim2.fromOffset(CornerLength, Thickness) })
			MakeLine(2, { Position = UDim2.fromOffset(Left + Width - CornerLength, Top), Size = UDim2.fromOffset(CornerLength, Thickness) })
			MakeLine(3, { Position = UDim2.fromOffset(Left, Top + Height), Size = UDim2.fromOffset(CornerLength, Thickness) })
			MakeLine(4, { Position = UDim2.fromOffset(Left + Width - CornerLength, Top + Height), Size = UDim2.fromOffset(CornerLength, Thickness) })
			MakeLine(5, { Position = UDim2.fromOffset(Left, Top), Size = UDim2.fromOffset(Thickness, CornerLength) })
			MakeLine(6, { Position = UDim2.fromOffset(Left + Width, Top), Size = UDim2.fromOffset(Thickness, CornerLength) })
			MakeLine(7, { Position = UDim2.fromOffset(Left, Top + Height - CornerLength), Size = UDim2.fromOffset(Thickness, CornerLength) })
			MakeLine(8, { Position = UDim2.fromOffset(Left + Width, Top + Height - CornerLength), Size = UDim2.fromOffset(Thickness, CornerLength) })
		end
	end

	function Handler:DrawHealthBar(Left: number, Top: number, Width: number, Height: number, Percent: number, Color: Color3, Transparency: number)
		local ScreenGui = self.Library.ScreenGui
	...
	function Handler:DrawName(Text: string, CenterX: number, TopY: number, Size: number, Color: Color3)
	...
	function Handler:DrawTracer(StartX: number, StartY: number, EndX: number, EndY: number, Color: Color3, Transparency: number)
		local ScreenGui = self.Library.ScreenGui
	...
	function Handler:UpdateChams(Player: Player, Enabled: boolean, Color: Color3?, Transparency: number?)
		if not Enabled then
	...
	function Handler:Destroy()
		...
		for _, Connection in Handler.Connections do
...
function ESP:AddObjectESP(Target: Instance, Options: ObjectESPOptions)
	...
	local function UpdateObjectESP()
		...
		if Opts.Box ~= false then
			...
			if ObjectSize then
				...
				local BottomPos = WorldToScreen(Position3D - Vector3World.new(0, ObjectSize.Y / 2, 0))
		...
		if Opts.Name ~= false then
			ObjectHandler:DrawName(DisplayName, ScreenPos.X, ScreenPos.Y - 14, 13, Opts.NameColor or Color3.fromRGB(255, 255, 0))
		end
		...
		if Opts.Distance ~= false then
			...
			ObjectHandler:DrawDistance(DistanceText, ScreenPos.X, ScreenPos.Y + 4, Opts.DistanceColor or Color3.fromRGB(200, 200, 200))
		end
		...
		if Opts.Tracer then
			...
			ObjectHandler:DrawTracer(StartX, StartY, ScreenPos.X, ScreenPos.Y, Opts.TracerColor or BoxColor, Opts.TracerTransparency or 0)
		end
		...
		if Opts.Highlight and Target:IsA("BasePart") then
			...
			ObjectHandler.HighlightInst.FillColor = Opts.HighlightColor or Color3.fromRGB(255, 255, 0)
	...
	function ObjectHandler:DrawBox(Left: number, Top: number, Width: number, Height: number, Color: Color3, Style: BoxStyle, Transparency: number, Thickness: number)
		local function MakeLine(Index: number, Props: { [string]: any })
			if self.BoxLines[Index] and self.BoxLines[Index].Parent then
				for K, V in Props do
					self.BoxLines[Index][K] = V
	...
	function ObjectHandler:DrawName(Text: string, CenterX: number, TopY: number, Size: number, Color: Color3)
		if not self.NameLabel or not self.NameLabel.Parent then
			if self.NameLabel then
				self.NameLabel:Destroy()
			end
	...
	function ObjectHandler:DrawDistance(Text: string, CenterX: number, BottomY: number, Color: Color3)
		if not self.DistanceLabel or not self.DistanceLabel.Parent then
			if self.DistanceLabel then
				self.DistanceLabel:Destroy()
			end
	...
	function ObjectHandler:DrawTracer(StartX: number, StartY: number, EndX: number, EndY: number, Color: Color3, Transparency: number)
		if not self.TracerLine or not self.TracerLine.Parent then
			if self.TracerLine then
				self.TracerLine:Destroy()
			end
...
function ESP:GetAimbotTarget(): (Player?, Vector3?)
	...
	for Player, Handler in self.Players do
		...
		local TargetPos = Opts.Prediction and PredictPosition(Character, HitPos, Opts.PredictionAmount) or HitPos
...
local function SetupSilentAim(self: any)
	...
	local SilentAimConnection = RunService.RenderStepped:Connect(function()
		...
		local LookCF = CFrame.lookAt(MyRoot.Position, Vector3.new(HitPos.X, MyRoot.Position.Y, HitPos.Z))
...
local function SetupAutoFire(self: any)
	...
	local AutoFireConnection = RunService.RenderStepped:Connect(function()
		...
		local ToolRemote = Tool:FindFirstChildWhichIsA("RemoteEvent") or Tool:FindFirstChildWhichIsA("RemoteFunction")
		if ToolRemote then
