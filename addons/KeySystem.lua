...
function KeySystem:Attach(KeyTab: any, Options: UIOptions?): UI
    ...
    local function ApplyTextAlignment()
        if typeof(Enum) ~= "table" or not Enum.TextXAlignment then
            return
        end

        local LabelAlignment = if Text.RightToLeft then Enum.TextXAlignment.Right else Enum.TextXAlignment.Center
        if PromptLabel and PromptLabel.TextLabel then
            PromptLabel.TextLabel.TextXAlignment = LabelAlignment
        end
        if StatusLabel and StatusLabel.TextLabel then
            StatusLabel.TextLabel.TextXAlignment = LabelAlignment
        end
        if KeyBox and KeyBox.TextBox then
            KeyBox.TextBox.TextXAlignment = if Text.RightToLeft
    ...
    function UI:UpdateText(NewOptions: UIOptions)
        ...
        if NewOptions.RightToLeft ~= nil then
            Text.RightToLeft = NewOptions.RightToLeft == true
        end
