...
function Movement.new(Library: any, Options: MovementOptions?)
    ...
    local self: any = setmetatable({
        ...
        InfiniteJump = false,
        AirControl = false,
        AirControlSpeed = Config.AirControlSpeed or 50,
        Spin = false,
        SpinSpeed = Config.SpinSpeed or 30,
        ClickTeleport = false,
        BlinkDistance = Config.BlinkDistance or 25,
        Connections = {},
        FlyBodyVelocity = nil,
        FlyBodyGyro = nil,
        OnGroundLast = false,
...
function Movement:_StartLoop()
    ...
    self._UpdateConnection = self:_GiveConnection(RunService.Heartbeat:Connect(function(DeltaTime)
        ...
        self:_UpdateAirControl(RootPart, Humanoid)
        self:_UpdateSpin(RootPart, DeltaTime)
    end))
end

function Movement:SetWalkSpeed(Speed: number, Enable: boolean?)
    self.WalkSpeed = math.clamp(Speed, 0, 1000)
    if Enable ~= nil then
...
function Movement:SetJumpPower(Power: number, Enable: boolean?)
    self.JumpPower = math.clamp(Power, 0, 1000)
    if Enable ~= nil then
...
function Movement:SetInfiniteJump(Enabled: boolean)
    self.InfiniteJump = Enabled

    if Enabled and not self._InfiniteJumpConnection then
        self._InfiniteJumpConnection =
            self:_GiveConnection(UserInputService.JumpRequest:Connect(function()
                if not self.InfiniteJump or self.Destroyed then
                    return
                end
                local Humanoid = GetHumanoid(Players.LocalPlayer)
                if Humanoid then
...
function Movement:SetAirControl(Enabled: boolean, Speed: number?)
    self.AirControl = Enabled
    if Speed then
        self.AirControlSpeed = math.max(1, Speed)
    end
end

function Movement:SetSpin(Enabled: boolean, Speed: number?)
    self.Spin = Enabled
    if Speed then
        self.SpinSpeed = math.max(1, Speed)
    end
end

function Movement:SetBlinkDistance(Distance: number)
    self.BlinkDistance = math.max(1, Distance)
end

function Movement:Blink(Distance: number?)
    local RootPart = GetRootPart(Players.LocalPlayer)
    if not RootPart then
        return
    end

    local Dist = Distance or self.BlinkDistance or 25
    local Camera = workspace.CurrentCamera
    local Dir = (Camera and Camera.CFrame.LookVector) or RootPart.CFrame.LookVector

    pcall(function()
        RootPart.CFrame = RootPart.CFrame + (Dir * Dist)
    end)
end

function Movement:SetClickTeleport(Enabled: boolean)
    self.ClickTeleport = Enabled

    if Enabled and not self._ClickTeleportConnection then
        self._ClickTeleportConnection =
            self:_GiveConnection(UserInputService.InputBegan:Connect(function(Input, GameProcessed)
                if not self.ClickTeleport or self.Destroyed then
                    return
                end
...
function Movement:_UpdateAirControl(RootPart: BasePart?, Humanoid: Humanoid?)
    if not self.AirControl or not RootPart or not RootPart.Parent or not Humanoid then
        return
    end
    ...
    pcall(function()
        ...
        if Dir.Magnitude > 0 then
            ...
            RootPart.Velocity =
                Vector3.new(Dir.X * self.AirControlSpeed, Vel.Y, Dir.Z * self.AirControlSpeed)
        end
    end)
end

function Movement:_UpdateSpin(RootPart: BasePart?, DeltaTime: number)
    if not self.Spin or not RootPart or not RootPart.Parent then
        return
    end

    pcall(function()
        RootPart.CFrame = RootPart.CFrame
            * CFrame.Angles(0, math.rad(self.SpinSpeed * DeltaTime), 0)
    end)
end
