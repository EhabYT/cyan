...
function QoL.new(Library: any, Options: QoLOptions?)
	...
	local function SetupConnections()
		...
		local function RefreshItemEsp()
			...
			for _, Part in workspace:GetDescendants() do
				if Part:IsA("BasePart") and Part.CanCollide == false and not Part:FindFirstAncestorOfClass("Player") and not Part:FindFirstAncestorOfClass("Tool") then
					local IsDroppedItem = Part:IsA("Part") or Part:IsA("MeshPart")
					if IsDroppedItem and (Part.Position - Origin).Magnitude <= self.Options.ItemEspRange then
		...
		local ACConnection = RunService.Heartbeat:Connect(function(Delta)
			...
			for _, Part in workspace:GetDescendants() do
				...
				if Part:FindFirstAncestorOfClass("Model") and Part.Parent:IsA("Model") and Part.Parent:FindFirstChildOfClass("Humanoid") then
					continue
				end
				...
				until (Root.Position - Part.Position).Magnitude < 4 or not Part.Parent or not BodyPos.Parent
	...
	local function SetupAutoClick()
					...
					if Tool then
						...
						if Handle then
							...
							if TouchInterest then
								firetouchinterest(Handle, (Players.LocalPlayer and Players.LocalPlayer.Character and Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart")) or Handle, 0)
							end
						end
					end
				end
		...
		table.insert(self.Connections, {Connected = true, Disconnect = function()
			if self.AutoClickThread then
				coroutine.close(self.AutoClickThread)
				self.AutoClickThread = nil
			end
...
function QoL:CopyServerLink()
	local PlaceId = game.PlaceId
	local JobId = game.JobId
	local Link = string.format("https://www.roblox.com/games/%d/Custom?gameInstanceId=%s", PlaceId, JobId)
	setclipboard(Link)
	return Link
end
