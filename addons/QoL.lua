...
function QoL:CopyJobId()
    local JobId = tostring(game.JobId)
    pcall(setclipboard, JobId)
    return JobId
end

function QoL:SetChatSpy(Enabled: boolean)
    self.ChatSpyEnabled = Enabled
    if Enabled then
        if not self._ChatSpyConnection then
            local ok, TCS = pcall(game.GetService, game, "TextChatService")
            if ok and TCS and TCS.MessageReceived then
                self._ChatSpyConnection = TCS.MessageReceived:Connect(function(Message)
                    local Name = (Message and Message.TextSource and Message.TextSource.Name) or "?"
                    local Text = (Message and Message.Text) or ""
                    self.Library:Notify({
                        Title = "[Chat] " .. tostring(Name),
                        Description = tostring(Text),
                        Time = 5,
                    })
                end)
            else
                local ok2, Chat = pcall(game.GetService, game, "Chat")
                if ok2 and Chat and Chat.Chatted then
                    self._ChatSpyConnection = Chat.Chatted:Connect(function(Text)
                        self.Library:Notify({
                            Title = "[Chat] You",
                            Description = tostring(Text),
                            Time = 5,
        ...
        if self._ChatSpyConnection then
            self._ChatSpyConnection:Disconnect()
            self._ChatSpyConnection = nil
        end
    end
end

function QoL:SetAutoRejoin(Enabled: boolean)
    self.AutoRejoinEnabled = Enabled
    if Enabled then
        if not self._AutoRejoinConnection then
            local PlaceId = game.PlaceId
            local JobId = game.JobId
            self._AutoRejoinConnection = RunService.Heartbeat:Connect(function()
                if not self.AutoRejoinEnabled then
                    return
                end
                local LP = Players.LocalPlayer
                if not LP or not LP:IsDescendantOf(Players) then
                    self.AutoRejoinEnabled = false
                    if self._AutoRejoinConnection then
                        self._AutoRejoinConnection:Disconnect()
                        self._AutoRejoinConnection = nil
                    end
                    pcall(function()
...
-- Notification toasts (player join / leave)
function QoL:SetNotificationsEnabled(Enabled: boolean)
    self.NotificationsEnabled = Enabled

    if Enabled then
        if not self._PlayerJoinConnection then
            self._PlayerJoinConnection = Players.PlayerAdded:Connect(function(Player)
                if not self.NotificationsEnabled then
                    return
                end
                self.Library:Notify({
                    Title = "Player Joined",
                    Description = tostring(Player.Name),
                    Time = self.NotificationDuration or 5,
                })
            end)
        end
        if not self._PlayerLeaveConnection then
            self._PlayerLeaveConnection = Players.PlayerRemoving:Connect(function(Player)
                if not self.NotificationsEnabled then
                    return
                end
                self.Library:Notify({
        ...
        if self._PlayerJoinConnection then
            self._PlayerJoinConnection:Disconnect()
            self._PlayerJoinConnection = nil
        end
        if self._PlayerLeaveConnection then
            self._PlayerLeaveConnection:Disconnect()
            self._PlayerLeaveConnection = nil
        end
    end
end

function QoL:SetNotificationDuration(Seconds: number)
    self.NotificationDuration = math.clamp(Seconds, 1, 30)
end

-- Best-effort performance tool: lower rendering quality for more FPS.
function QoL:SetGraphicsQuality(Value: number)
    self.GraphicsQuality = math.clamp(Value, 1, 10)
    pcall(function()
        local U = UserSettings()
        if U and U.GameSettings then
            U.GameSettings.GraphicsQuality = self.GraphicsQuality
        end
    end)
end
