...
function SaveManager:SetLocalization(Localizer: any, Prefix: string?)
    assert(
    ...
    SaveManager.Localization = Localizer
    SaveManager.LocalizationPrefix = Prefix or "save"
end

local function Translate(Key: string, Fallback: string, Parameters: { [string]: any }?): string
    local Localizer = SaveManager.Localization
    if not Localizer then
        return Fallback
    end

    local FullKey = SaveManager.LocalizationPrefix .. "." .. Key
...
function SaveManager:BuildConfigSection(Tab: any, IconName: string)
    ...
    local ConfigurationBox = Tab:AddRightGroupbox(Translate("group", "Configuration"), IconName or "folder-cog")
    BindText(ConfigurationBox, "group")
    ...
    if not Supported then
        ConfigurationBox:AddLabel(
            Translate(
                ...
                "Configuration persistence is unavailable: " .. tostring(SupportError),
                {
    ...
    local function AddButton(Key: string, Fallback: string, Callback: () -> ())
        return BindText(ConfigurationBox:AddButton(Translate(Key, Fallback), Callback), Key)
    end
    local function AddActionButton(Key: string, Fallback: string, Info: any)
        Info.Text = Translate(Key, Fallback)
        return BindText(ConfigurationBox:AddButton(Info), Key)
    end
    ...
    local function RefreshAutoloadConfigLabel()
        ...
        AutoloadConfigLabel:SetText(
            Translate("current_autoload", string.format("Current autoload config: %s", AutoloadConfigName), {
                name = AutoloadConfigName,
            })
        )
        if ConfigList and not ConfigList.Destroyed then
            RefreshList()
        end
    end
    ...
    BindText(
        ConfigurationBox:AddInput("SaveManager_ConfigName", {
            Text = Translate("config_name", "Config name"),
        }),
        "config_name"
    )

    AddButton("create_config", "Create config", function()
        ...
        ShowDialog(
            ...
            Translate("config_exists", "Config already exists"),
            Translate(
                "config_exists_description",
                string.format(
    ...
    BindText(
        ConfigurationBox:AddDropdown("SaveManager_ConfigList", {
            Text = Translate("config_list", "Config list"),
            ...
            FormatDisplayValue = function(Value: any)
                if Value == SaveManager.AutoloadConfig then
                    return Translate("autoload_suffix", string.format("%s (autoload)", Value), { name = Value })
                end
    ...
    AddActionButton("load_config", "Load config", {
        DoubleClick = false,
    ...
    AddActionButton("overwrite_config", "Overwrite config", {
        DoubleClick = false,
        ...
        Func = function()
            ...
            ShowDialog(
                ...
                Translate("overwrite_config_title", "Overwrite config"),
                Translate(
                    "overwrite_config_description",
                    string.format(
    ...
    AddActionButton("rename_config", "Rename config", {
        DoubleClick = false,
    ...
    AddActionButton("delete_config", "Delete config", {
        DoubleClick = false,
        ...
        Func = function()
            ...
            ShowDialog(
                ...
                Translate("delete_config_title", "Delete config"),
                Translate(
    ...
    --// Autoload Config
    AddActionButton("set_autoload", "Set as autoload", {
        DoubleClick = false,
    ...
    AddActionButton("reset_autoload", "Reset autoload", {
        DoubleClick = false,
        ...
        Func = function()
            ShowDialog(
                ...
                Translate("reset_autoload_title", "Reset autoload config"),
                Translate(
                    "reset_autoload_description",
    ...
    AutoloadConfigLabel =
        ConfigurationBox:AddLabel(Translate("current_autoload", "Current autoload config: ...", { name = "..." }), true)
    ...
    if SaveManager.Localization and typeof(SaveManager.Localization.OnChanged) == "function" then
        SaveManager.Localization:OnChanged(function()
            if AutoloadConfigLabel and not AutoloadConfigLabel.Destroyed then
                RefreshAutoloadConfigLabel()
            end
        end)
    end
