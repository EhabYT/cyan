...
-- Best-effort skybox presets. Asset IDs are content links; if a game already
-- has a custom sky our "Default" preset simply removes Cyan's Sky so the
-- place's original sky shows through.
local SKYBOX_PRESETS = {
    Default = nil,
    Night = {
        Top = "rbxassetid://970760225",
        Bottom = "rbxassetid://970758921",
        Left = "rbxassetid://970759776",
        Right = "rbxassetid://970760964",
        Front = "rbxassetid://970759279",
        Back = "rbxassetid://970758706",
    },
    Brick = {
        Top = "rbxassetid://11709431",
        Bottom = "rbxassetid://11709329",
        Left = "rbxassetid://11709386",
        Right = "rbxassetid://11709411",
        Front = "rbxassetid://11709400",
        Back = "rbxassetid://11709361",
    },
}
...
function Visuals.new(Library: any, Options: VisualsOptions?)
    ...
    SkyInstance.Name = "CyanSky"
    SkyInstance.Archivable = false
    ...
    local self: any = setmetatable({
        ...
        FPSCounter = false,
        SkyboxName = Config.SkyboxName or "Default",
        ClockTime = Config.ClockTime or SavedClockTime or 12,
        AmbientColor = Config.Ambient,
        NoFog = false,
        FPSCounterDisplay = "",
        FPSTable = {},
        FPSIndex = 0,
    ...
    if Config.SkyboxName and Config.SkyboxName ~= "Default" then
        self:SetSkybox(Config.SkyboxName)
    end
    if Config.ClockTime then
        self:SetClockTime(Config.ClockTime)
    end
    if Config.Ambient then
        self:SetAmbient(Config.Ambient)
    end
...
function Visuals:SetSkybox(Name: string)
    self.SkyboxName = Name or "Default"

    if self.SkyboxName == "Default" then
        if SkyInstance and SkyInstance.Parent then
            SkyInstance.Parent = nil
        end
        return
    end

    local Preset = SKYBOX_PRESETS[self.SkyboxName]
    if not Preset then
        return
    end

    if not SkyInstance.Parent then
        SkyInstance.Parent = Lighting
    end

    SkyInstance.SkyboxTop = Preset.Top
    SkyInstance.SkyboxBottom = Preset.Bottom
    SkyInstance.SkyboxLeft = Preset.Left
    SkyInstance.SkyboxRight = Preset.Right
    SkyInstance.SkyboxFront = Preset.Front
    SkyInstance.SkyboxBack = Preset.Back
end

function Visuals:SetClockTime(Hour: number)
    if Hour == nil then
        return
    end

    self.ClockTime = math.clamp(Hour, 0, 24)
    Lighting.ClockTime = self.ClockTime
end

function Visuals:SetAmbient(Color: Color3)
...
function Visuals:SetNoFog(Enabled: boolean)
    self.NoFog = Enabled
...
function Visuals:ResetAll()
    ...
    self:SetXRay(false)
    self:SetSkybox("Default")
    self:SetClockTime(self.SavedLighting.ClockTime or 12)
    self:SetAmbient(self.SavedLighting.Ambient or Color3.new(0.5, 0.5, 0.5))
    self:SetNoFog(false)
    self:ResetFog()
