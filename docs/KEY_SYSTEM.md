...
`Attach()` returns a UI binding with `UpdateText(options)` for live localization of the prompt, placeholder, idle/checking/verified/rejected/locked/logout statuses, and reset button. It keeps the visible status synchronized with the current key-system state:
