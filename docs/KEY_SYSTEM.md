...
This controls the UI flow only. Server code must still validate and enforce all protected actions. `GateTabs` returns a controller with `Lock()` and `Unlock()` methods for an explicit logout or re-authentication flow.
