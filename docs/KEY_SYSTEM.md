...
`Attach()` returns a UI binding with `UpdateText(options)` for live localization of the prompt, placeholder, idle/checking/verified/rejected/locked/logout statuses, and reset button. Pass `RightToLeft = true` to align the login labels and key entry for RTL locales. It keeps the visible status synchronized with the current key-system state:

```luau
local KeyTab = Window:AddKeyTab("Access", "key")
local KeyUI = Gate:Attach(KeyTab, { Prompt = "Enter your access key." })

KeyUI:UpdateText({
    Prompt = "أدخل مفتاح الدخول.",
    IdleText = "الحالة: في انتظار مفتاح",
    ResetText = "إعادة تعيين إدخال المفتاح",
})
```
